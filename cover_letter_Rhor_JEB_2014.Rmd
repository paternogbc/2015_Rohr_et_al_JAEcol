...
mat <- read.csv("data_med_max.csv",h=T,sep=",")
mat <- melt(mat, id.vars = c("sp", "DF"),
...
c_data_max <- comparative.data(phy=filo,data=filter(mat,source=="max"),names.col="sp",vcv=T,vcv.dim=3)
c_data_med <- comparative.data(phy=filo,data=filter(mat,source=="med"),names.col="sp",vcv=T,vcv.dim=3)
```
...
------------------
```{r}
g1 <- ggplot(mat,aes(y=log10(DF),x=log10(SVL),colour=source))+
          geom_point()+
